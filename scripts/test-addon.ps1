...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlRag-tests"
}
...
if (Test-WindowsHost) {
	...
	Write-Step "Configuring and running ofxGgmlRag tests with Visual Studio tools"
	Invoke-CheckedCmd "ofxGgmlRag tests" $command
} else {
	Write-Step "Configuring ofxGgmlRag tests"
	Invoke-CheckedNative "cmake configure ofxGgmlRag tests" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration
	}
	Write-Step "Building ofxGgmlRag tests"
	Invoke-CheckedNative "cmake build ofxGgmlRag tests" {
		cmake --build $BuildDir --config $Configuration
	}
	Write-Step "Running ofxGgmlRag tests"
	Invoke-CheckedNative "ctest ofxGgmlRag tests" {
