param()

$ErrorActionPreference = "Stop"

function Write-Step {
	param([string]$Message)
	Write-Host "==> $Message"
}

function Assert-Path {
	param(
		[string]$Path,
		[string]$Label,
		[switch]$Directory
	)

	if ($Directory) {
		if (!(Test-Path -LiteralPath $Path -PathType Container)) {
			throw "$Label was not found: $Path"
		}
	} elseif (!(Test-Path -LiteralPath $Path -PathType Leaf)) {
		throw "$Label was not found: $Path"
	}
}

function Assert-FileContains {
	param(
		[string]$Path,
		[string]$Pattern,
		[string]$Label
	)

	$content = Get-Content -LiteralPath $Path -Raw
	if ($content -notmatch $Pattern) {
		throw "$Label did not contain expected pattern: $Pattern"
	}
}
function Assert-FileNotContains {
	param(
		[string]$Path,
		[string]$Pattern,
		[string]$Label
	)

	$content = Get-Content -LiteralPath $Path -Raw
	if ($content -match $Pattern) {
		throw "$Label contained forbidden pattern: $Pattern"
	}
}
$scriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$addonRoot = Split-Path -Parent $scriptRoot
$addonsRoot = Split-Path -Parent $addonRoot

Write-Step "Checking addon skeleton"
Assert-Path (Join-Path $addonRoot "addon_config.mk") "addon config"
Assert-FileNotContains (Join-Path $addonRoot "addon_config.mk") "ADDON_DEPENDENCIES.*ofxGgmlCore" "addon config"
Assert-Path (Join-Path $addonRoot "README.md") "README"
Assert-Path (Join-Path $addonRoot "LICENSE") "license"
Assert-Path (Join-Path $addonRoot "CHANGELOG.md") "changelog"
Assert-Path (Join-Path $addonRoot "docs\GETTING_STARTED.md") "getting started docs"
Assert-Path (Join-Path $addonRoot "docs\RAG_WORKFLOWS.md") "RAG workflow docs"
Assert-Path (Join-Path $addonRoot "docs\releases\v1.1.0-draft.md") "draft release notes"
Assert-FileContains (Join-Path $addonRoot "CHANGELOG.md") "Unreleased" "changelog"
Assert-FileContains (Join-Path $addonRoot "CHANGELOG.md") "loadTextCorpus" "changelog"
Assert-FileContains (Join-Path $addonRoot "README.md") "docs/RAG_WORKFLOWS.md" "README"
Assert-FileContains (Join-Path $addonRoot "README.md") "docs/GETTING_STARTED.md" "README"
Assert-FileContains (Join-Path $addonRoot "docs\GETTING_STARTED.md") "run-rag-runtime-smoke.ps1" "getting started docs"
Assert-FileContains (Join-Path $addonRoot "docs\GETTING_STARTED.md") "OFXGGML_RAG_SOURCE_ROOT" "getting started docs"
Assert-FileContains (Join-Path $addonRoot "docs\GETTING_STARTED.md") "ModelBacked.*false" "getting started docs"
Assert-FileContains (Join-Path $addonRoot "docs\releases\v1.1.0-draft.md") "TextCorpusBridge=true" "draft release notes"
Assert-FileContains (Join-Path $addonRoot "docs\releases\v1.1.0-draft.md") "ModelBacked=false" "draft release notes"
Assert-FileContains (Join-Path $addonRoot "docs\RAG_WORKFLOWS.md") "Planning handoff" "RAG workflow docs"
Assert-FileContains (Join-Path $addonRoot "docs\RAG_WORKFLOWS.md") "Validation ladder" "RAG workflow docs"
Assert-FileContains (Join-Path $addonRoot "docs\RAG_WORKFLOWS.md") "generated artifacts" "RAG workflow docs"
Assert-Path (Join-Path $addonRoot "src\ofxGgmlRag.h") "public header"
Assert-Path (Join-Path $addonRoot "src\ofxGgmlRagVersion.h") "version header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag.h") "ofxGgmlRagVersion.h" "public header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag.h") "class ofxGgmlRag" "public header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag.h") "clearRetrievalCache" "public header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag.h") "buildPrompt" "public header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag.h") "draftAnswer" "public header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag.h") "findCitations" "public header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagUtils.h") "searchEmbeddedChunks" "utility header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagUtils.h") "refinedQueries" "utility header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagUtils.h") "findCitationsFromInput" "utility header"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRagVersion.h") "OFXGGML_RAG_VERSION_STRING" "version header"
Assert-Path (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagTypes.h") "types header"
Assert-Path (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRag.cpp") "facade source"
Assert-Path (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagUtils.h") "utility header"
Assert-Path (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagUtils.cpp") "utility source"
Assert-Path (Join-Path $addonRoot "scripts\run-model-backed-rag-smoke.ps1") "model-backed RAG smoke"
Assert-Path (Join-Path $addonRoot "scripts\run-model-backed-rag-smoke.bat") "model-backed RAG smoke launcher"
Assert-FileContains (Join-Path $addonRoot "README.md") "run-model-backed-rag-smoke" "README model-backed smoke"

Write-Step "Checking dependency layout"
Assert-Path (Join-Path $addonsRoot "ofxImGui") "sibling ofxImGui addon for examples" -Directory

Write-Step "Checking example layout"
$exampleRoot = Join-Path $addonRoot "ofxGgmlRagSearchExample"
Assert-Path $exampleRoot "root-level smoke example" -Directory
Assert-Path (Join-Path $exampleRoot "addons.make") "smoke example addons.make"
Assert-Path (Join-Path $exampleRoot "config.make") "smoke example config.make"
Assert-FileNotContains (Join-Path $exampleRoot "addons.make") "(?m)^ofxGgmlCore\s*$" "smoke example addons.make"
Assert-FileContains (Join-Path $exampleRoot "addons.make") "(?m)^ofxGgmlRag\s*$" "smoke example addons.make"
Assert-FileContains (Join-Path $exampleRoot "addons.make") "(?m)^ofxImGui\s*$" "smoke example addons.make"
Assert-FileContains (Join-Path $exampleRoot "config.make") "OFXIMGUI_GLFW_EVENTS_REPLACE_OF_CALLBACKS=0" "smoke example config.make"
Assert-Path (Join-Path $exampleRoot "src\main.cpp") "smoke example main.cpp"
Assert-Path (Join-Path $exampleRoot "src\ofApp.h") "smoke example ofApp.h"
Assert-Path (Join-Path $exampleRoot "src\ofApp.cpp") "smoke example ofApp.cpp"
Assert-Path (Join-Path $exampleRoot "README.md") "smoke example README"
Assert-Path (Join-Path $exampleRoot "src\WebSearchRunner.cpp") "consolidated web runner"
Assert-Path (Join-Path $exampleRoot "src\WebSearchSupport.cpp") "consolidated web support"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "rag\.retrieve" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "LLM Prompt" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Answer Draft" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Citations" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Variants" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Quality rank" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "cache=%s" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.h") "ofxImGui" "smoke example ofApp.h"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "OFXGGML_RAG_SOURCE_ROOT" "smoke example ofApp.cpp"
Assert-FileContains (Join-Path $exampleRoot "README.md") "OFXGGML_RAG_SOURCE_ROOT" "smoke example README"
Assert-FileContains (Join-Path $exampleRoot "README.md") "llama-server" "smoke example canonical backend"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Live web" "smoke example web mode"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Person / quotes" "smoke example quote mode"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "IsKeyChordPressed.*ImGuiMod_Ctrl.*ImGuiKey_V" "smoke example Ctrl+V path"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "Paste##" "smoke example explicit paste control"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'ofSystemLoadDialog\("Select a local GGUF text model"' "smoke example local model browser"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'start-llama-server\.ps1' "smoke example local llama-server launcher"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'OFXGGML_LLAMA_SERVER_READY=1' "smoke example stable llama-server readiness contract"
Assert-FileNotContains (Join-Path $exampleRoot "src\ofApp.cpp") 'Reusing the existing server' "smoke example human-readable launcher parsing"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'std::async' "smoke example asynchronous local model start"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'webSearchLaunch.*std::async' "smoke example asynchronous web retrieval"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'Searching\.\.\.' "smoke example visible web progress"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'inputTextWithPaste\("Model alias"' "smoke example compatible server alias"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") 'diversifyRetrieval' "smoke example source-diverse web retrieval"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") 'Quote attributed to' "smoke example quote-grounded model context"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") '"max_tokens"' "smoke example bounded model answer"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") 'total fetch budget' "smoke example bounded aggregate scraping"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") 'modelTimeoutSeconds' "smoke example separate model timeout"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") 'searchEmbeddedChunks' "smoke example local embedding rerank"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") 'response_format' "smoke example strict structured answer"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'Select a local GGUF embedding model' "smoke example embedding model browser"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'const bool modelServerStartDisabled = modelServerStarting;[\s\S]*if \(modelServerStartDisabled\) ImGui::BeginDisabled\(\);[\s\S]*if \(modelServerStartDisabled\) ImGui::EndDisabled\(\);' "smoke example balanced local model disabled scope"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'const bool embeddingServerStartDisabled = embeddingServerStarting;[\s\S]*if \(embeddingServerStartDisabled\) ImGui::BeginDisabled\(\);[\s\S]*if \(embeddingServerStartDisabled\) ImGui::EndDisabled\(\);' "smoke example balanced embedding model disabled scope"
Assert-FileNotContains (Join-Path $exampleRoot "src\ofApp.cpp") 'if \((modelServerStarting|embeddingServerStarting)\) ImGui::EndDisabled\(\);' "smoke example mutable disabled-scope close condition"
Assert-FileContains (Join-Path $exampleRoot "src\WebSearchRunner.cpp") "MODEL SUMMARY.*NOT A QUOTE" "smoke example quote/model distinction"
Assert-FileContains (Join-Path $addonRoot "src\ofxGgmlRag\ofxGgmlRagUtils.h") "retrieveTextCorpus" "utility header"
Assert-Path (Join-Path $addonRoot "tests\CMakeLists.txt") "test CMakeLists"
Assert-Path (Join-Path $addonRoot "tests\test_main.cpp") "test source"
Assert-Path (Join-Path $scriptRoot "doctor-rag.ps1") "RAG doctor script"
Assert-Path (Join-Path $scriptRoot "doctor-rag.bat") "RAG doctor Windows wrapper"
Assert-Path (Join-Path $scriptRoot "doctor-rag.sh") "RAG doctor shell wrapper"
Assert-Path (Join-Path $scriptRoot "test-doctor-rag.ps1") "RAG doctor smoke test"
Assert-Path (Join-Path $scriptRoot "run-rag-runtime-smoke.ps1") "RAG runtime smoke script"
Assert-Path (Join-Path $scriptRoot "run-rag-runtime-smoke.bat") "RAG runtime smoke Windows wrapper"
Assert-Path (Join-Path $scriptRoot "run-rag-runtime-smoke.sh") "RAG runtime smoke shell wrapper"
Assert-Path (Join-Path $scriptRoot "test-rag-runtime-smoke.ps1") "RAG runtime smoke contract test"

$nestedExamples = Join-Path $addonRoot "examples"
if (Test-Path -LiteralPath $nestedExamples -PathType Container) {
	throw "Examples should live at the addon root, not under: $nestedExamples"
}
$separateWebExample = Join-Path $addonRoot "ofxGgmlRagWebSearchExample"
if (Test-Path -LiteralPath $separateWebExample) {
	throw "Local and web RAG should remain consolidated in ofxGgmlRagSearchExample"
}

Write-Step "Checking generated artifact hygiene"
$forbidden = @(
	"build",
	".vs",
	"ofxGgmlRagSearchExample\bin",
	"ofxGgmlRagSearchExample\obj",
	"ofxGgmlRagSearchExample\.vs",
	"models"
)

foreach ($relative in $forbidden) {
	$pathspec = $relative -replace "\\", "/"
	$tracked = git -C $addonRoot ls-files -- $pathspec
	if ($LASTEXITCODE -ne 0) {
		throw "Failed to inspect generated artifact path: $relative"
	}
	if ($tracked) {
		throw "Generated or local-only path should not be committed here: $relative"
	}
}

Write-Step "Checking RAG doctor"
& (Join-Path $scriptRoot "test-doctor-rag.ps1")
if (!$?) {
	throw "RAG doctor smoke test failed"
}

Write-Step "Checking RAG runtime smoke contract"
& (Join-Path $scriptRoot "test-rag-runtime-smoke.ps1")

Write-Step "Running headless tests"
& (Join-Path $scriptRoot "test-addon.ps1") -Clean
if ($LASTEXITCODE -ne 0) {
	throw "Headless tests failed with exit code $LASTEXITCODE"
}

Write-Step "ofxGgmlRag local validation passed"
